...
本 ADR 的 MCP 绑定决策仍然有效；独立 TTS 服务、消息朗读和 Voice Mode 复用 TTS 的部分
已删除且不保留兼容路径。当前语音决策见 `009-qwen-audio-realtime-voice.md`。
...
- 变更类 MCP 工具在引入人工确认、权限校验和审计前不得加入默认
  `enableTools` 允许列表。
- MCP 工具参数和结果不进入公共 NDJSON。文本聊天使用协议 v3 的受控 `tool_status`
  展示进度：内部服务器名、工具名和调用标识先映射为公共业务操作和顺序标识，未知工具
  统一降为“外部业务数据”，前端只显示运行、完成或失败状态。
