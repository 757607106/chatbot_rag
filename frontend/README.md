...
该变量只在 Next.js 服务端读取，不会暴露给浏览器。知识库页面无需登录，通过
`/api/knowledge/*` BFF 访问 Python 管理 API。
...
聊天页面保持原有布局，左侧栏新增“知识库”入口；知识库页面可直接进入，提供知识库创建
与切换、文档管理、单切片编辑、版本回滚和召回测试。管理面不提供内置身份验证，只适用于
本地开发或受信网络。
